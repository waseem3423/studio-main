...
import { generateSalesReport } from "@/app/(app)/reports/actions";
import { CSVLink } from "react-csv";
import { useToast } from "@/hooks/use-toast";
import type { EnrichedSale } from "@/app/(app)/reports/actions";
import { format } from "date-fns";

interface SalesReportDownloaderProps {
  startDate?: Date;
  endDate?: Date;
  onValidate: () => boolean;
}
...
export function SalesReportDownloader({ startDate, endDate, onValidate }: SalesReportDownloaderProps) {
  ...
  const handleGenerateReport = async () => {
    ...
    const result = await generateSalesReport({ message: "", data: undefined }, formData);
    ...
    sales.forEach(sale => {
      sale.products.forEach(product => {
        data.push({
          ...
          "Salesman": sale.salesmanName || "N/A",
    ...
    <>
      ...
      {readyToDownload && (
        <CSVLink
          data={reportData}
          headers={csvHeaders}
          filename={`Sales_Report_${format(startDate!, 'yyyy-MM-dd')}_to_${format(endDate!, 'yyyy-MM-dd')}.csv`}
          className="w-full"
          target="_blank"
        >
